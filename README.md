...
OpenRank 是一个综合性的工具套件，用于分析 Python 开源项目的环境依赖、健康度和社区友好度。通过数据爬取、静态分析和 AI 辅助分析，帮助开发者快速评估项目的工程成熟度。
...
│   ├── step1_repo_dependency_overview.py  # 依赖文件缺失分析
...
│   ├── github_token.txt    # GitHub API Token (需自行配置)
│   ├── data/               # 分析结果数据
...
- **仓库采样**: 从 GitHub 按 star 分桶随机采样 Python 项目
